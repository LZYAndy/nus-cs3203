...
bool AffectsStarCompute::is_affects_star(PKB& pkb, int assignment1, int assignment2, bool called_by_affects_star)
{
    if (called_by_affects_star)
    {
        std::unordered_set<std::string> states;
        return is_affects_star_helper(pkb, assignment1, assignment2, states);
    }
    else if (!affects_star_cache.empty())
    {
    ...
    {
        get_all_affects_star_relationship(pkb);
        std::vector<int> affected = affects_star_cache.get(assignment1);
        if (find(affected.begin(), affected.end(), assignment2) != affected.end())
        {
            return true;
...
// affects*(1,a)
std::vector<int> AffectsStarCompute::get_affected_star(PKB& pkb, int assignment, bool called_from_all_relationship)
{
    if (called_from_all_relationship)
    {
        vector<int> result;
        vector<int> assign_stmts = pkb.get_all_assigns();
        for (int assign_stmt : assign_stmts)
        {
            if (is_affects_star(pkb, assignment, assign_stmt, true))
            {
...
std::unordered_map<int, std::vector<int>> AffectsStarCompute::get_all_affects_star_relationship(PKB &pkb)
{
    if (affects_star_cache.empty())
    {
        ...
        for (int assign_stmt : assign_stmts)
        {
            std::vector<int> affected_stmts = get_affected_star(pkb, assign_stmt, true);
    ...
    {
        return affects_star_cache.copy();
    }
}
