        ...
        pql_dto::Entity &second_param, PKB &PKB)
{
    ...
    if (first_param.is_entity_declared())
    {
        ...
        {
            // e.g. Calls*(p, q)
            unordered_map<string, vector<string>> str_map = PKB.get_all_procedures_calls_star_relationship();
    ...
    if (second_param.is_entity_declared())
    {
        ...
        {
            // e.g. Calls*("main", q)
            vector<string> str_vec = PKB.get_procedures_called_by_star(first_name);
            result = QueryUtility::mapping(second_param, str_vec);
        }
    }
    return result;
        ...
        pql_dto::Entity &second_param, PKB &PKB)
{
    ...
    if (QueryUtility::is_proc_name(first_param))
    {
        ...
        {
            // e.g. Calls*("main", _)
            result = !PKB.get_procedures_called_by_star(first_name).empty();
        }
        else if (QueryUtility::is_proc_name(second_param))
        {
            // e.g. Calls("main", "sub")
