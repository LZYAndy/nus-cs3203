#include "NewQueryEvaluator.h"

#include <utility>

unordered_set<string> NewQueryEvaluator::get_result(string &query, PKB &PKB)
{
    string error_msg;
    Cache cache;
    bool is_select_bool;
    ...
    {
        bool bool_clauses_result = NewQueryEvaluator::evaluateBoolGroup(no_synonym_clauses, PKB);
        if (!bool_clauses_result)
        {
            return NewQueryEvaluator::evaluateEmptyMap(is_select_bool);
        }
    }
    ...
    if (!synonyms_not_in_select_clauses.empty())
    {
        ...
        {
            is_satisfy = NewQueryEvaluator::evaluateGroup(group, PKB, cache);
            if (!is_satisfy)
            {
                return NewQueryEvaluator::evaluateEmptyMap(is_select_bool);
            }
    ...
    // Select synonyms
    result = NewQueryEvaluator::select(select_clause, select_map, final_map, PKB);
    return result;
}

unordered_set<string> NewQueryEvaluator::select(vector<pql_dto::Entity> &select_clause,
        map<string, vector<string>> &select_map,
        unordered_map<string, vector<string>> &final_map,
        PKB &PKB)
{
    ...
    { // if the select type is BOOLEAN
        if (NewQueryEvaluator::is_empty_map(final_map))
        {
            return unordered_set<string> ({"TRUE"});
        }
    }
    // if the select type is tuple
    unordered_set<string> common_select_synonyms = NewQueryEvaluator::get_common_synonyms(final_map, select_map);
    ...
    for (const auto& synonym : select_map)
    {
        ...
        if (common_select_synonyms.find(synonym.first) != common_select_synonyms.end() && is_first_common_synonym)
        {
            ...
            {
                temp_map[cs] = QueryUtility::change_to_attributes(select_clause.at(NewQueryEvaluator::get_element_index_in_map(select_map, cs)), select_map.at(cs), PKB);
            }
        }
        ...
        {
            temp_map[synonym.first] = QueryUtility::change_to_attributes(select_clause.at(NewQueryEvaluator::get_element_index_in_map(select_map, synonym.first)), synonym.second, PKB);
        }
        acc_map = NewQueryEvaluator::merge_two_maps(temp_map, acc_map, NewQueryEvaluator::get_common_synonyms(temp_map, acc_map));
    }
...
unordered_map<string, vector<string>> NewQueryEvaluator::mergeGroup(vector<pql_dto::Constraint> &group, Cache &cache)
{
    ...
    for (pql_dto::Constraint clause : group)
    {
        ...
        unordered_set<string> common_synonyms = NewQueryEvaluator::get_common_synonyms(result, intermediary_map);
        result = NewQueryEvaluator::merge_two_maps(result, intermediary_map, common_synonyms);
        if (NewQueryEvaluator::is_empty_map(result))
        {
            return empty_map;
        }
    }
...
bool NewQueryEvaluator::evaluateBoolGroup(deque<pql_dto::Constraint> group, PKB &PKB)
{
    // can only be such that clauses
    bool result;
    for (pql_dto::Constraint clause : group)
    {
        Relationships relation = clause.get_relationship();
        result = NewQueryEvaluator::evaluateSuchThat(relation, PKB).at(0);
        if (!result)
        {
            return false;
        }
...
bool NewQueryEvaluator::evaluateGroup(deque<pql_dto::Constraint> group, PKB &PKB, Cache &cache)
{
    unordered_map<string, vector<string>> empty_map;
    unordered_map<string, vector<string>> intermediary_map;
    ...
    for (pql_dto::Constraint clause : group)
    {
        ...
        {
            Relationships relation = clause.get_relationship();
            intermediary_map = NewQueryEvaluator::evaluateSuchThat(relation, PKB, cache).at(1);
        }
        else if (clause.is_pattern())
        {
            Pattern pattern = clause.get_pattern();
            intermediary_map = NewQueryEvaluator::evaluatePattern(pattern, PKB, cache);
        }
        else if (clause.is_with())
        {
            With with = clause.get_with();
            intermediary_map = NewQueryEvaluator::evaluateWith(with, PKB, cache);
        }
        if (NewQueryEvaluator::is_empty_map(intermediary_map))
        {
            return false;
        }
    }
...
unordered_set<string> NewQueryEvaluator::evaluateEmptyMap(bool is_select_bool)
{
    if (is_select_bool)
    {
        return unordered_set<string> {"FALSE"};
    ...
    if (relation_type == RelationshipType::FOLLOWS)
    {
        if (!relation.is_relationship_star())
        {
            if (!first_param.is_entity_declared() && !second_param.is_entity_declared())
            {
                ...
                return vector<bool, unordered_map<string, vector<string>>> {trivial, empty_map};
            }
            else
            {
...
int NewQueryEvaluator::get_element_index_in_map(map<string, vector<string>> &map, string key)
{
    int i = 0;
    int result = 0;
...
bool NewQueryEvaluator::is_empty_map(unordered_map<string, vector<string>> &map)
{
    if (map.empty())
    {
        return true;
...
unordered_map<string, vector<string>> NewQueryEvaluator::merge_two_maps(unordered_map<string, vector<string>> map_1,
        unordered_map<string, vector<string>> map_2, unordered_set<string> common_part)
{
    ...
    { // no common synonyms, cross product
        if (NewQueryEvaluator::is_empty_map(map_1))
        {
            return map_2;
        }
        if (NewQueryEvaluator::is_empty_map(map_2))
        {
            return map_1;
        }
...
unordered_set<string> NewQueryEvaluator::get_common_part(vector<string> &str_vec_1, vector<string> &str_vec_2)
{
    unordered_set<string> result;
