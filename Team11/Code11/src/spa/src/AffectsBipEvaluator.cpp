...
unordered_map<string, vector<string>> AffectsBipEvaluator::evaluate_non_trivial(pql_dto::Entity &first_param,
        pql_dto::Entity &second_param, PKB &PKB)
{
    ...
    if (first_param.is_entity_declared())
    {
        ...
        {
            // e.g. AffectsBip(a, a)
            unordered_map<int, vector<int>> int_map = PKB.get_all_affects_bip_relationship();
    ...
    if (second_param.is_entity_declared())
    {
        ...
        {
            // e.g. AffectsBip(1, a)
            vector<int> int_vec = PKB.get_assigns_affected_bip_by(stoi(first_name));
            result = QueryUtility::mapping(second_param, int_vec, PKB);
        }
    }
    return result;
...
bool AffectsBipEvaluator::evaluate_trivial(pql_dto::Entity &first_param,
        pql_dto::Entity &second_param, PKB &PKB)
{
    bool result = false;
    string first_name = first_param.get_entity_name();
    ...
    if (QueryUtility::is_statement_num(first_param))
    {
        ...
        {
            // e.g. AffectsBip(1, _)
            result = !PKB.get_assigns_affected_bip_by(stoi(first_name)).empty();
        }
        else if (QueryUtility::is_statement_num(second_param))
        {
            // e.g. AffectsBip(1, 2)
            result = PKB.is_affects_bip(stoi(first_name), stoi(second_name));
        }
    }
    return result;
}
